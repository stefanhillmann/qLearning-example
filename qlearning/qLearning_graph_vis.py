...
def draw_q_graph():
    ...
    for i in states:
        for j in states:
            if q_matrix[i][j] > 0.0:
                q_graph.add_edge(i, j, q=round(q_matrix[i][j], 2))

    pos = nx.spring_layout(q_graph)
    nx.draw_networkx(q_graph, pos)
    edge_labels = nx.get_edge_attributes(q_graph, 'q')
    nx.draw_networkx_edge_labels(q_graph, pos, labels=edge_labels)
